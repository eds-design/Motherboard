<form method="POST" action="<?= BASE_URL ?>/settings" class="settings-saveable-form px-6 py-4" data-tab="printout">
    ...
    <div class="space-y-6">
        ...
        <div class="space-y-3">
            ...
            <div class="flex items-start">
                <input id="print_condensed" name="print_condensed" type="checkbox" value="1" <?= ($settings['print_condensed'] ?? '0') === '1' ? 'checked' : '' ?> class="h-4 w-4 mt-0.5 text-primary-600 focus:ring-primary-500 border-gray-300 rounded">
                <label for="print_condensed" class="ml-2 block text-sm text-gray-700"><?= t('settings.print_condensed') ?></label>
            </div>
        </div>
    </div>
